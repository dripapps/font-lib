...
class File extends BinaryStream
{
    ...
    {
        $fontFamily = $this->getNameTableString(name::NAME_PREFERRE_FAMILY);
    ...
    function getFontWeight(): string
    {
        ...
        if ($this->getTableObject("OS/2")->data ?? null) {
            $usWeightClass = $this->getTableObject("OS/2")->data["usWeightClass"];
            if ($usWeightClass >= 700) {
                $weight = 'bold';
            }
        } else {
            $fullName = $this->getFontFullName();

            if ($fullName) {
                $fullName = strtolower($fullName);

                if (str_contains($fullName, 'thin')) {
                    $weight = 'thin';
                } elseif (str_contains($fullName, 'extra-light')) {
                    $weight = 'extra-light';
                } elseif (str_contains($fullName, 'light')) {
                    $weight = 'light';
                } elseif (str_contains($fullName, 'medium')) {
                    $weight = 'medium';
                } elseif (str_contains($fullName, 'semi-bold')) {
                    $weight = 'semi-bold';
                } elseif (str_contains($fullName, 'bold')) {
                    $weight = 'bold';
                } elseif (str_contains($fullName, 'extra-bold')) {
                    $weight = 'extra-bold';
                } elseif (str_contains($fullName, 'black')) {
                    $weight = 'black';
                }
            }
        }
    ...
    function getFontStyle(): string
    {
        ...
        if ($this->getTableObject("OS/2")->data ?? null) {
            if ($this->getTableObject("OS/2")->data["fsSelection"] & 1) {
                $style = 'italic';
            }

            if ($this->getTableObject("OS/2")->data["fsSelection"] & 32) {
                $style = 'oblique';
            }
        } else {
            $fontFullName = $this->getFontFullName();

            if ($fontFullName) {
                $fontFullName = strtolower($fontFullName);
                if (str_contains($fontFullName, 'italic')) {
                    $style = 'italic';
                } elseif (str_contains($fontFullName, 'oblique')) {
                    $style = 'oblique';
                }
            }
        }
